...
export const LeftSidebar: React.FC<LeftSidebarProps> = ({
  ...
  // Рассчитываем прогресс для каждой сессии
  const calculateProgress = (session: SessionItem) => {
    if (session.total_sentences > 0 && session.current_position > 0) {
      return Math.round((session.current_position / session.total_sentences) * 100);
    }
    return 0;
  };
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-gray-400 text-[10px]">
                          ({session.current_position}/{session.total_sentences || '?'})
                        </span>
                      </div>
                    </div>
